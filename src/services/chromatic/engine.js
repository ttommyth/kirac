...
//orginal code from https://github.com/Siveran/siveran.github.io/blob/master/PoEChromaticCalc.js
function engine(){
  ...
  Main.calculate = function(options) {
    var probs = [];
    var error = false;
    var socks =options.sockets;// Std.parseInt(options.sockets);
    var str = options.str;//Std.parseInt(options.str);
    var dex =options.dex;// Std.parseInt(options.dex);
    var $int =options.int;// Std.parseInt(options.int);
    var red = options.r;//Std.parseInt(options.r);
    var green = options.g;//Std.parseInt(options.g);
    var blue = options.b;//Std.parseInt(options.b);
    if(str == null) str = 0;
    if(dex == null) dex = 0;
    if($int == null) $int = 0;
    if(socks == null) {
      socks = red + blue + green;
      options.sockets = + socks;
    }
    if(socks <= 0 || socks > 6) {
      error = true;
